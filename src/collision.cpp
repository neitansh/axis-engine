...
static bool add_area_node_boxes(const v3s16 min, const v3s16 max, ICrateDef *cratedef,
		Environment *env, std::vector<NearbyCollisionInfo> &cinfo)
{
	const auto *nodedef = cratedef->getNodeDefManager();
...
CollisionMoveResult collisionMoveSimple(Environment *env, ICrateDef *cratedef,
		const aabb3f &box_0,
		...
		StepUpMode step_up_mode)
{
	...
	*/
	...
	cinfo.clear();
	{
		...
		bool any_position_valid = add_area_node_boxes(min, max, cratedef, env, cinfo);
...
bool collision_check_intersection(Environment *env, ICrateDef *cratedef,
