...
void set_default_settings()
{
	...
	settings->setDefault("menu_ui_sound_volume", "0.15");
	...
	settings->setDefault("leaves_detail_range", "0");
	...
	// Умолчание — привычный крест с просветом: он не закрывает цель. Обводка
	// по умолчанию выключена: она выручает на пёстром фоне, но утолщает штрихи
	// и в спокойной картинке только мешает — кому нужна, тот её включит.
	...
	settings->setDefault("crosshair_outline", "0");
	...
	settings->setDefault("main_menu_rml", "true");

	// General font settings
	settings->setDefault("font_path", porting::getDataPath("fonts" DIR_DELIM "DepartureMono-Regular.otf"));
	settings->setDefault("font_path_italic", porting::getDataPath("fonts" DIR_DELIM "DepartureMono-Regular.otf"));
	settings->setDefault("font_path_bold", porting::getDataPath("fonts" DIR_DELIM "DepartureMono-Regular.otf"));
	settings->setDefault("font_path_bold_italic", porting::getDataPath("fonts" DIR_DELIM "DepartureMono-Regular.otf"));
	...
	// Тем же лесам: картинка облика с этой машины, вместо приезжающей по
	// хэшу из билета. Нужна затем, что дорогу «раздать — дождаться — назвать»
	// иначе можно пройти только на боевом сервере.
	settings->setDefault("player_avatars_test_skin", "");
