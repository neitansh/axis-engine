...
//! Ударная волна: то, что взрыв делает с уже лежащим мусором.
/*!
 * Частица живёт на клиенте, и сервер её не знает — послать ей толчок как
 * сущности нельзя. Но взрыв, от которого крошка на полу не шелохнулась,
 * выглядит хуже, чем взрыв вовсе без крошки: поле боя оказывается
 * нарисованным, а не разбитым.
 *
 * Поэтому волна посылается не частице, а месту: «в такой-то точке рвануло с
 * такой-то силой». Клиент сам находит свои частицы в этом шаре и раскидывает
 * их. Дёшево (один обход списка на взрыв), не требует от сервера знать про
 * мусор ничего и работает одинаково для всего, что этот мусор породило.
 *
 * Волна поднимает только те частицы, которые признают мир — те, у кого
 * `collisiondetection`. Дым, следы на земле и отметины от пуль сквозь мир
 * проходят, и разносить их взрывом незачем: отпечаток ноги не сдувает.
 */
struct ParticleShockwave
{
	//! Где рвануло, в узлах.
	v3f pos;
	//! Докуда достаёт, в узлах. За краем шара не трогает никого.
	f32 radius = 5.0f;
	//! Сила у самого центра, в узлах в секунду. К краю сходит на нет.
	f32 strength = 12.0f;
	//! Добавка вверх. Волна поднимает лежащее, а не возит его по полу.
	f32 lift = 6.0f;
	//! Разнобой направления, доля от единицы. Ноль — все летят строго от
	//! центра, и разлёт читается как строй.
	f32 spread = 0.5f;
};
