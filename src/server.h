...
public:
	...
	*/
	...
	/// Одеть игроков заново: приехал чей-то облик, и до сих пор его носитель
	/// ходил в том, что везёт сам движок.
	void refreshAvatars();

	/// Номер для медиа, которое добавляет сам движок, а не мод.
	///
	/// Модам номера раздаются случайными; наши идут подряд от верхнего края и
	/// сверяются с занятыми — так две раздачи не встанут на один номер.
	u32 allocateEngineMediaToken();

	/// Наше ли это медиа. У модового номера на той стороне ждёт колбэк, у
	/// нашего — никого, и звать по нему Lua нельзя: она падает.
	bool isEngineMediaToken(u32 token) const
	{
		return token == 0 || m_engine_media_tokens.count(token) > 0;
	}

	/// Ждёт ли ещё кто-нибудь этот файл. Пока ждёт — называть его в свойствах
	/// объекта рано: клиент подставит заглушку и второй раз не переспросит.
	bool mediaPending(u32 token) const { return m_pending_dyn_media.count(token) > 0; }
	...
	bool SendEyelids(session_t peer_id, f32 closed, f32 fade,
...
private:
	...
	*/
	...
	SkinCache m_skins;
	u32 m_engine_media_token = U32_MAX;
	std::unordered_set<u32> m_engine_media_tokens;
