...
enum ToClientCommand : u16
{
	...
	*/
	...
	TOCLIENT_EYELIDS = 0x68,
	/*
		Веки игрока: глаза закрываются, на чёрном проявляется подпись, глаза
		открываются — экран смерти. Считает и рисует клиент; сервер называет
		только цель и за сколько к ней прийти, так что смыкание и раскрытие
		плавные при любой частоте кадров.
