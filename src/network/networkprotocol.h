...
// крейт это 4783 файла и 17 МБ.
#define MEDIA_MAX_FILES (32768U)
#define MEDIA_MAX_TOTAL_SIZE (512U * 1024U * 1024U)

// Набор медиа кусками: тысяча поездок за мелкими файлами дороже самих файлов
// (см. src/server/mediahttp.cpp), но и одним куском на двадцать мегабайт его
// возить нельзя — на слабом канале оборванная закачка теряет всё разом и не
// показывает игроку никакого движения.
//
// Поэтому кусок — сам себе архив: пришёл, распаковался, разложился по местам.
// Потерялся — стоит только себя, остальное уже сохранено, а чего не хватает,
// доберётся поштучно.
//
// Внутри куска записи вида «хэш (40 символов), длина (u32), данные», и всё это
// сжато zstd. По адресу без номера лежит опись: сколько кусков и сколько
// файлов всего.
#define MEDIA_BUNDLE_SIGNATURE 0x41584d42 // 'AXMB'
#define MEDIA_BUNDLE_VERSION 2
#define MEDIA_BUNDLE_FILE_NAME "bundle"
// Сколько несжатых данных класть в кусок. Два мегабайта доезжают и по слабому
// каналу, и их не жалко потерять при обрыве.
#define MEDIA_BUNDLE_PART_SIZE (2U * 1024U * 1024U)

// Что вообще бывает медиа. Список один на сервер и клиент нарочно: сервер
// отбирает по нему файлы крейта, клиент — то, что ему объявили. Разойдись они,
...
enum ToClientCommand : u16
{
	...
	*/
	...
	TOCLIENT_CAMERA_IMPULSE = 0x67,
	/*
		Толчок камере: отдача, удар взрывной волны, дрожь.

		Смысл пакета — в том, чего он НЕ делает. Он не трогает поворот игрока
		и не шлёт MOVE_PLAYER, поэтому не перезаписывает накопители мышиного
		обзора и не возвращает позицию к серверной. Камере добавляется
		смещение поверх обзора, которое клиент сам приводит к нулю; мышь всё
		это время работает как обычно.

		Интегрирует клиент, а не сервер: слать угол каждый тик значило бы
		двадцать ступенек в секунду вместо плавного движения. Сервер посылает
		один импульс на событие, дальше считает CameraFx.

		Клиенту старше версии протокола 54 этот пакет не отправляется вовсе.

		u8 kind
		kind == 0 (отдача):
			f32 pitch, yaw, roll   // рад/с, оси как у обзора игрока
			f32 stiffness, damping
		kind == 1 (удар):
			f32 pitch, yaw, roll   // рад/с
			f32 x, y, z            // ноды/с, горизонтальная система игрока
			f32 stiffness, damping
		kind == 2 (дрожь):
			f32 amplitude          // рад
			f32 frequency          // Гц
	...
	*/
	...
	TOCLIENT_SCREEN_STATIC = 0x68,
	/*
		Помехи на экране: шум потерянного сигнала, размытие, виньетка и
		подпись посередине — экран смерти. Считает и рисует клиент; сервер
		называет только цель и за сколько к ней прийти, так что появление и
		уход плавные при любой частоте кадров.

		Клиенту старше версии протокола 56 не отправляется.

		f32 intensity          // 0 — чисто, 1 — сплошные помехи
		f32 fade               // секунд до цели
		std::string caption    // подпись, UTF-8; пустая — без подписи
	*/
