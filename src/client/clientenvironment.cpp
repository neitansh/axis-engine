...
void ClientEnvironment::step(float dtime)
{
	...
	*/
	...
	// С оборванной связью замирают не только чужие объекты, но и сам игрок,
	// и по той же причине: сервер помнит его там, где его застало молчание, и
	// на возвращении поставит ровно туда. Всё, что он прошёл бы за это время —
	// по инерции, под гравитацией, шагом с обрыва, — пришлось бы отматывать
	// рывком, а падение в пропасть отматывать уже нечем. Осматриваться это не
	// мешает: камера шага не спрашивает.
	if (!m_client->isLinkLive()) {
		lplayer->setSpeed(v3f(0.0f));
		steps = 0;
	}
		...
		if (!free_move) {
			...
			if (lplayer->move_resistance > 0) {
				...
				if (in_liquid_stable) {
					...
					// единицу move_resistance.
					//
					// Ходьба глушится не втрое слабее падения, а вдвое: при
					// втрое слабее сквозь листву шли почти как по полю, и
					// заметить сопротивление можно было только падая. Разница
					// между осями всё равно нужна — гравитация разгоняет
					// вшестеро сильнее, чем разгоняется шаг, — но не такая.
					//
					// При семёрке, самой густой среде, какую позволяет
					// описание ноды, падение садится до 2.3 нод/с, шаг до
					// 0.7 нод/с.
					static const f32 drag_fall = 1.2f;
					static const f32 drag_walk = 0.6f;
