...
public:
	...
	/**
	 * Собирает конвейер отрисовки заново по текущим настройкам.
	 *
	 * Из каких шагов состоит конвейер, решается один раз при его сборке: свечение,
	 * объёмный свет, сглаживание и масштаб кадра - это не переключатели внутри
	 * шагов, а сами шаги, которых либо нет, либо они есть. Настройку, меняющую
	 * состав, поэтому приходится отрабатывать пересборкой; звать её нужно между
	 * кадрами, а не посреди отрисовки.
	 *
	 * @return true, если теневой рендерер пришлось создать заново - тогда он
	 * пуст, и объекты должны заявить о себе ему снова
	 */
	/**
	 * Собрать конвейер заново.
	 *
	 * @param reset_shadows завести рендерер теней с нуля. Нужно, когда
	 * поменялись его собственные числа — размер карты, дальность: они
	 * читаются один раз, при создании, и переехавший в новый конвейер старый
	 * рендерер остался бы со старыми.
	 */
	bool rebuildPipeline(Client *client, Hud *hud, bool reset_shadows = false);
