...
LocalPlayer::LocalPlayer(Client *client, const std::string &name) : Player(name, client->idef()),
																	m_client(client)
{
	...
	*/
	if (could_sneak && m_sneak_node_exists)
	{
		...
		if (y_diff > 0 && m_speed.Y <= 0.0f)
		{
			...
			if (y_diff < sneak_stepheight || (physics_override.sneak_glitch && !collision_check_intersection(env, m_client, m_collisionbox, check_pos, m_cao)))
			{
...
void LocalPlayer::applyControl(float dtime, Environment *env)
{
	...
	bool is_sprinting = control.sprint || (free_move && physics_override.speed > 1.1f);
	bool fast_climb = fast_move && is_sprinting && !player_settings.aux1_descends;
	...
	if (superspeed || (is_climbing && fast_climb) ||
		...
		speedH = speedH.normalize() * speed_walk * 2.5f;
	else
...
float LocalPlayer::getSlipFactor(Environment *env, const v3f &speedH)
{
	...
	const NodeDefManager *nodemgr = env->getCrateDef()->ndef();
	Map *map = &env->getMap();
	const ContentFeatures &f = nodemgr->get(map->getNode(getStandingNodePos()));
	int slippery = 0;
	if (f.walkable)
