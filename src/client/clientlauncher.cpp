...
static void dump_start_data(const GameStartData &data)
{
	std::cout <<
		...
		"\nworld game  " << data.world_spec.crateid <<
		"\ngame path   " << data.crate_spec.path <<
								 ...
								 const Settings &cmd_args)
{
	...
	if (start_data.isAnyServer())
	{
		...
		// Note that start_data.crate_spec contains the crateid from the command line
		...
		if (!start_data.crate_spec.isValid())
		{
			...
			if (world_exists)
			{
				msg = gettext("Could not find or load game: ") + worldspec.crateid;
			}
			else
			{
