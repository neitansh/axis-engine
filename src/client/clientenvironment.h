...
public:
	...
	Client *getCrateDef() { return m_client; }
	...
	*/
	...
	/**
	 * Помечает облик каждого объекта устаревшим, чтобы он был создан заново.
	 *
	 * Нужно, когда объект должен заново заявить о себе тому, что живёт вне его
	 * самого. Так устроены тени: список отбрасывающих их узлов наполняют сами
	 * объекты, когда появляются в мире, и включённые посреди игры тени застали
	 * бы этот список пустым.
	 */
	void expireObjectVisuals();
