...
void Particle::step(float dtime, ClientEnvironment *env)
{
	...
	if (vanishes_on_touch) {
		...
		/*
		 * Мусор, которому велено улечься, ложится с первого касания земли.
		 *
		 * Ждать, пока он остановится сам, нельзя: трения у частиц нет, и
		 * упавший обломок скользит по земле вечно — а значит считается каждый
		 * кадр, ради чего всё и затевалось наоборот. Мелкий обломок и не
		 * должен скакать: куда упал, там и лежит.
		 */
