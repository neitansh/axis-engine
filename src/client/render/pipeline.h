...
public:
	...
	/**
	 * Уточнение к имени шага.
	 *
	 * Шагов одного типа в конвейере может быть десяток - вся постобработка
	 * состоит из них, - и в профиле они складывались бы в одну строку, где не
	 * видно, что дороже: выделение ярких мест, уменьшение или сведение.
	 * Шаг, у которого есть чем себя назвать, возвращает это здесь.
	 */
	virtual std::string getStepLabel() const { return {}; }
...
/// Сколько участков зарегистрировано
u32 getGpuSlotCount();

/**
 * Выбрать участок, у которого в этом кадре считаются вызовы пиксельного
 * шейдера.
 *
 * Аппаратный счётчик один, и вложенные запросы OpenGL запрещает, поэтому
 * участки меряются по очереди: за десяток кадров набирается вся картина, а
 * стоит это ровно один запрос за кадр.
 */
