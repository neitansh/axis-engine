...
class Client : public con::PeerHandler, public InventoryManager, public ICrateDef
{
public:
	...
	*/
	...
	void handleCommand_Eyelids(NetworkPacket *pkt);
	...
	*/
	...
	/**
	 * Ставит в очередь пересборку мешей всех загруженных блоков.
	 *
	 * Нужно, когда настройка меняет то, из чего меш складывается: сглаженный
	 * свет, отражения на воде. Работа уходит в фоновые потоки, так что мир
	 * обновляется не мгновенно, а по мере готовности - зато не замирает.
	 */
	void rebuildAllMeshes();

	/**
	 * Строит визуалы нод заново и пересобирает по ним карту.
	 *
	 * Настройки вида листвы, стекла и размера текстур решают не то, как нода
	 * рисуется шейдером, а то, из чего она вообще состоит, и записано это в
	 * визуалах, которые строятся один раз на сессию.
	 */
	void rebuildNodeVisuals();
