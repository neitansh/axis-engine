...
void IMoveAction::apply(InventoryManager *mgr, ServerActiveObject *player, ICrateDef *cratedef)
{
	...
	*/
	if (!ignore_rollback && cratedef->rollback()) {
		IRollbackManager *rollback = cratedef->rollback();
...
void IMoveAction::clientApply(InventoryManager *mgr, ICrateDef *cratedef)
{
...
void IDropAction::apply(InventoryManager *mgr, ServerActiveObject *player, ICrateDef *cratedef)
{
	Inventory *inv_from = mgr->getInventory(from_inv);
	...
	*/
	...
	/*
		Record rollback information
	*/
	if (!ignore_src_rollback && cratedef->rollback()) {
		IRollbackManager *rollback = cratedef->rollback();
...
void IDropAction::clientApply(InventoryManager *mgr, ICrateDef *cratedef)
{
	// Optional InventoryAction operation that is run on the client
	// to make lag less apparent.
...
void ICraftAction::apply(InventoryManager *mgr,
	ServerActiveObject *player, ICrateDef *cratedef)
{
	...
	*/
	...
	getCraftingResult(inv_craft, crafted, output_replacements, false, cratedef);
	...
	while (found && list_craftresult->itemFits(0, crafted)) {
		...
		getCraftingResult(inv_craft, crafted, temp, true, cratedef);
		...
		IItemDefManager *itemdef = cratedef->getItemDefManager();
		...
		getCraftingResult(inv_craft, crafted, temp, false, cratedef);
		PLAYER_TO_SA(player)->item_CraftPredict(crafted, player, list_craft, craft_inv);
		found = !crafted.empty();
	}
...
void ICraftAction::clientApply(InventoryManager *mgr, ICrateDef *cratedef)
{
	// Optional InventoryAction operation that is run on the client
	// to make lag less apparent.
}
...
bool getCraftingResult(Inventory *inv, ItemStack &result,
		std::vector<ItemStack> &output_replacements,
		bool decrementInput, ICrateDef *cratedef)
{
	result.clear();
	...
	CraftOutput co;
	bool found = cratedef->getCraftDefManager()->getCraftResult(
			ci, co, output_replacements, decrementInput, cratedef);
	if (found)
		result.deSerialize(co.item, cratedef->getItemDefManager());
