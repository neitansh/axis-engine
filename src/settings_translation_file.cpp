...
fake_function() {
	...
	gettext("Sprint key");
	gettext("Key for sprinting (speeding up walking and flying).");
	...
	gettext("Zoom");
	gettext("Pick item");
	gettext("Key for picking up the block you are looking at.\nSee https://docs.luanti.org/for-players/controls/");
	gettext("Look left");
	gettext("Look right");
	gettext("Look up");
	gettext("Look down");
	...
	gettext("View distance in chunks (1 chunk = 16 blocks).\nMin = 2");
	...
	gettext("Volumetric clouds");
	gettext("Clouds you can fly into: a layer traced through by the view, instead of a\nflat sheet overhead. Replaces the flat clouds while enabled.");
	gettext("Cloud height");
	gettext("Height of the cloud layer, in nodes.");
	gettext("Cloud density");
	gettext("How much of the sky the clouds cover. 0 is a few stray wisps, 1 is overcast.");
	gettext("Cloud speed");
	gettext("How fast the clouds drift.");
	gettext("Cloud distance");
	gettext("Distance at which the clouds dissolve into the sky, in nodes. Clouds meet a\nlowered view far away, so this has to be considerably larger than the\nviewing range for them to reach the horizon.");
	gettext("Cloud size");
	gettext("Size of a single cloud, in nodes. Small clouds seen from a distance blur\ninto noise, so a cloud should be considerably larger than a building.");
	gettext("Cloud square size");
	gettext("Side of the squares a cloud is built from, in nodes. Cloud size and cloud\nsquare size are different things: the first says how large a cloud is, the\nsecond how coarsely it is cut.");
	gettext("Cloud thickness");
	gettext("Thickness of the cloud layer, in nodes.");
	gettext("Cloud edge roundness");
	gettext("Shape of the cloud edges. 0 keeps them blocky, 0.5 makes them smooth.");
	...
	gettext("Crosshair shape");
	gettext("Shape the crosshair is built from.\nOnly used when no crosshair.png is provided by a texture pack.");
	gettext("Crosshair size");
	gettext("Length of each crosshair stroke, in pixels.");
	gettext("Crosshair thickness");
	gettext("Thickness of the crosshair lines, in pixels.");
	gettext("Crosshair gap");
	gettext("Gap between the centre and where the strokes begin, in pixels.");
	gettext("Crosshair centre dot");
	gettext("Size of the dot in the centre, in pixels. Zero for no dot.");
	gettext("Crosshair outline");
	gettext("Thickness of the dark outline around the crosshair, in pixels.\nHelps a white crosshair stand out against snow and sky, at the cost of\nthicker strokes. Zero for none.");
	gettext("Crosshair outline color");
	gettext("Crosshair outline color (R,G,B).");
	gettext("Crosshair outline alpha");
	gettext("Crosshair outline alpha (opaqueness, between 0 and 255).");
	gettext("Crosshair color on target");
	gettext("Crosshair color while pointing at a player or entity (R,G,B).");
	gettext("GUI");
	...
	gettext("Name tags shown at once");
	gettext("How many name tags may be shown at once. The ones nearest to the crosshair\nwin; the rest are not drawn at all. Zero means no limit.\nA crate may narrow this further through its own rules, but not widen it.");
	...
	gettext("Load all installed mods");
	gettext("Load every mod found in the mods folder and nothing else: a mod that is\nthere runs, a mod that is gone does not. Worlds keep no mod list at all.\nTurn this off to let each world pick its mods again.");
	gettext("Network motion diagnostics");
	gettext("Show what network motion is doing: how evenly packets arrive, how deep the\nplayback buffer runs, and how much the speed of what you see changes from\nframe to frame - which is what shaking is.");
	gettext("Log network motion to CSV");
	gettext("Also write the measurements to netsync/frames.csv and netsync/packets.csv\nin the user directory, for looking at afterwards.\nRequires \"Network motion diagnostics\".");
	gettext("Wait for the server to return");
	gettext("When a server stops answering, keep the world on screen and wait for it to\ncome back instead of returning to the main menu. Restarts and short\noutages are then barely noticeable.\nThe world is reloaded once the server answers again.");
	gettext("Give up waiting after");
	gettext("Give up waiting for a silent server after this many seconds and return to\nthe main menu. 0 waits for as long as the player lets it.\nRequires \"Wait for the server to return\".");
	...
	gettext("Emulated network latency");
	gettext("Hold incoming packets back by this many milliseconds, as a distant server\nwould. For measuring how motion looks on a worse connection than the one\nyou have; 0 turns the whole emulation off.\nOnly affects what this client receives. Nothing is done to what it sends,\nand no server is affected.");
	gettext("Emulated network jitter");
	gettext("Vary the emulated latency by up to this many milliseconds either way,\nwhich is what reorders packets on a real link.");
	gettext("Emulated packet loss");
	gettext("Throw away this percentage of incoming packets.");
	gettext("Emulated network seed");
	gettext("Seed for the emulated network, so that two measurements meet the same\npattern of jitter and loss and can be compared.");
	gettext("Server");
	gettext("Admin name");
	gettext("Name of the player.\nWhen running a server, a client connecting with this name is admin.\nWhen starting from the main menu, this is overridden.");
	gettext("Main world name");
	gettext("World a dedicated server runs, taken from the worlds folder.\nA world of that name is created if it does not exist yet.\nThe \"--worldname\" and \"--world\" arguments take precedence over this.");
	...
	gettext("TCP port the server hands out its own media on, next to the game port.\nMedia over the game protocol crawls while the reliable window ramps up;\nover HTTP the client takes files in dozens of connections at once and the\nfirst join stops being a wait. Empty means the same number as the game\nport, 0 turns the whole thing off. Single player never opens it.");
	gettext("IPv6 server");
	...
	gettext("Screenshot after");
	gettext("Take a screenshot this many seconds after entering a world, then quit.\nMeant for adjusting how the game looks in repeated runs, without a\nperson at the keyboard. 0 disables it.");
	...
	gettext("Run the main menu on RmlUi instead of the Lua menu.\nOff — the old Lua menu with the content tab and per-world mod selection.");
