...
ServerMap::ServerMap(const std::string &savedir, ICrateDef *cratedef,
		...
		ServerEnvironment *env, u32 liquid_loop_max)
{
	...
	while (!liquid_queue.empty())
	{
		...
		*/
		...
		std::string suspect;
		if (m_cratedef->rollback())
			suspect = m_cratedef->rollback()->getSuspect(p0, 83, 1);

		if (m_cratedef->rollback() && !suspect.empty()) {
			// Blame suspect
			RollbackScopeActor rollback_scope(m_cratedef->rollback(), suspect, true);
			// Get old node for rollback
			RollbackNode rollback_oldnode(this, p0, m_cratedef);
			// Set node
			setNode(p0, n0);
			// Report
			RollbackNode rollback_newnode(this, p0, m_cratedef);
			RollbackAction action;
			action.setSetNode(p0, rollback_oldnode, rollback_newnode);
			m_cratedef->rollback()->reportAction(action);
		} else {
			// Set node
			setNode(p0, n0);
		}
