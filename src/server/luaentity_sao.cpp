...
void LuaEntitySAO::sendPosition(bool do_interpolate, bool is_movement_end)
{
	...
	/*
	 * Обычная посылка идёт ненадёжно: потерянную поправит следующая, а
	 * задержанная стоила бы дороже потерянной.
	 *
	 * Закрывающая — нет. Ею объект говорит, где он встал, и следующей не
	 * будет: улёгшийся обломок засыпает и до скончания века молчит. Потеряв
	 * её, клиент остаётся с последним, что слышал, — местом посреди полёта, —
	 * и рисует крошку висящей в воздухе, хотя на сервере она давно лежит.
	 * Именно так и выглядел мусор, парящий над воронкой: карты сходились до
	 * узла, расходились места.
	 *
	 * Таких посылок мало — по одной на улёгшийся объект, — и надёжность им по
	 * средствам.
	 */
	m_messages_out.emplace(getId(), is_movement_end, str);
}
