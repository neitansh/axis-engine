...
public:
	...
	*/
	...
	virtual bool check(const CraftInput &input, ICrateDef *cratedef) const=0;
	...
	virtual CraftOutput getOutput(const CraftInput &input, ICrateDef *cratedef) const=0;
	// the inverse of the above
	virtual CraftInput getInput(const CraftOutput &output, ICrateDef *cratedef) const=0;
	// Decreases count of every input item
	virtual void decrementInput(CraftInput &input,
		std::vector<ItemStack> &output_replacements, ICrateDef *cratedef) const=0;
	...
	virtual void initHash(ICrateDef *cratedef) = 0;
...
public:
	...
	virtual bool check(const CraftInput &input, ICrateDef *cratedef) const;
	virtual CraftOutput getOutput(const CraftInput &input, ICrateDef *cratedef) const;
	virtual CraftInput getInput(const CraftOutput &output, ICrateDef *cratedef) const;
	virtual void decrementInput(CraftInput &input,
		std::vector<ItemStack> &output_replacements, ICrateDef *cratedef) const;

	virtual u64 getHash(CraftHashType type) const;

	virtual void initHash(ICrateDef *cratedef);
...
public:
	...
	virtual bool getCraftResult(CraftInput &input, CraftOutput &output,
			...
			bool decrementInput, ICrateDef *cratedef) const=0;

	virtual std::vector<CraftDefinition*> getCraftRecipes(CraftOutput &output,
			ICrateDef *cratedef, unsigned limit=0) const=0;
...
public:
	...
	virtual bool clearCraftsByOutput(const CraftOutput &output, ICrateDef *cratedef) = 0;
	virtual bool clearCraftsByInput(const CraftInput &input, ICrateDef *cratedef) = 0;
	...
	virtual void registerCraft(CraftDefinition *def, ICrateDef *cratedef) = 0;

	// Delete all crafting definitions
	virtual void clear()=0;

	// To be called after all mods are loaded, so that we catch all aliases
	virtual void initHashes(ICrateDef *cratedef) = 0;
};
