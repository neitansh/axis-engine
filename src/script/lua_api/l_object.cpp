...
static scene::TrackId read_track_id(lua_State *L, int index)
{
	...
	{
		throw LuaError("Track number out of range (1 to " + std::to_string(U16_MAX + 1) + ")");
	}
...
int ObjectRef::l_set_properties(lua_State *L)
{
	...
	const auto old = *prop;
	read_object_properties(L, 2, sao, prop, getCrateDef(L)->idef());
	if (*prop != old)
	{
		prop->validate();
...
luaL_Reg ObjectRef::methods[] = {
	...
	luamethod(ObjectRef, set_eyelids),
