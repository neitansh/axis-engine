...
int ModApiCraft::l_register_craft(lua_State *L)
{
	...
	*/
	if(type == "shaped"){
		...
		craftdef->registerCraft(def, getCrateDef(L));
	}
	/*
...
int ModApiCraft::l_clear_craft(lua_State *L)
{
	...
	if (!output.empty()) {
		...
		if (craftdef->clearCraftsByOutput(c_output, getCrateDef(L))) {
	...
	for (const auto &item : recipe) {
		items.emplace_back(item, 1, 0, getCrateDef(L)->idef());
	}
	CraftInput input(method, width, items);

	if (!craftdef->clearCraftsByInput(input, getCrateDef(L))) {
...
static void push_craft_recipe(lua_State *L, ICrateDef *gdef,
