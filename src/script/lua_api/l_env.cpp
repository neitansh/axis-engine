...
int ModApiEnv::l_get_node_max_level(lua_State *L)
{
	...
	v3s16 pos = read_v3s16(L, 1);
	MapNode n = env->getMap().getNode(pos);
	lua_pushnumber(L, n.getMaxLevel(env->getCrateDef()->ndef()));
	return 1;
}
...
int ModApiEnv::l_get_node_level(lua_State *L)
{
	...
	v3s16 pos = read_v3s16(L, 1);
	MapNode n = env->getMap().getNode(pos);
	lua_pushnumber(L, n.getLevel(env->getCrateDef()->ndef()));
	return 1;
}
...
int ModApiEnv::l_set_node_level(lua_State *L)
{
	...
	MapNode n = env->getMap().getNode(pos);
	lua_pushnumber(L, n.setLevel(env->getCrateDef()->ndef(), level));
	env->swapNode(pos, n);
	return 1;
}
...
int ModApiEnv::l_add_node_level(lua_State *L)
{
	...
	if(lua_isnumber(L, 2))
		level = lua_tonumber(L, 2);
	MapNode n = env->getMap().getNode(pos);
	lua_pushnumber(L, n.addLevel(env->getCrateDef()->ndef(), level));
	env->swapNode(pos, n);
	return 1;
}
...
int ModApiEnv::l_add_item(lua_State *L)
{
	...
	//v3f pos = checkFloatPos(L, 1);
	// item
	ItemStack item = read_item(L, 2, getCrateDef(L)->idef());
	if(item.empty() || !item.isKnown(getCrateDef(L)->idef()))
		return 0;
...
int ModApiEnv::l_find_path(lua_State *L)
{
	...
	std::vector<v3s16> path = get_path(&env->getServerMap(), env->getCrateDef()->ndef(), pos1, pos2,
...
int ModApiEnvVM::l_get_node_max_level(lua_State *L)
{
	...
	lua_pushnumber(L, n.getMaxLevel(getCrateDef(L)->ndef()));
	return 1;
}
...
int ModApiEnvVM::l_get_node_level(lua_State *L)
{
	...
	v3s16 pos = read_v3s16(L, 1);
	MapNode n = vm->getNodeNoExNoEmerge(pos);
	lua_pushnumber(L, n.getLevel(getCrateDef(L)->ndef()));
	return 1;
}
...
int ModApiEnvVM::l_set_node_level(lua_State *L)
{
	...
	if (lua_isnumber(L, 2))
		level = lua_tonumber(L, 2);
	MapNode n = vm->getNodeNoExNoEmerge(pos);
	lua_pushnumber(L, n.setLevel(getCrateDef(L)->ndef(), level));
...
int ModApiEnvVM::l_add_node_level(lua_State *L)
{
	...
	if (lua_isnumber(L, 2))
		level = lua_tonumber(L, 2);
	MapNode n = vm->getNodeNoExNoEmerge(pos);
	lua_pushnumber(L, n.addLevel(getCrateDef(L)->ndef(), level));
	vm->setNodeNoEmerge(pos, n);
	return 1;
}
