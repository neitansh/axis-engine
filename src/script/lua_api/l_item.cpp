...
int LuaItemStack::l_get_description(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	LuaItemStack *o = checkObject<LuaItemStack>(L, 1);
	std::string desc = o->m_stack.getDescription(getCrateDef(L)->idef());
...
int LuaItemStack::l_get_short_description(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	LuaItemStack *o = checkObject<LuaItemStack>(L, 1);
	std::string desc = o->m_stack.getShortDescription(getCrateDef(L)->idef());
	lua_pushstring(L, desc.c_str());
	return 1;
}
...
int LuaItemStack::l_replace(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	LuaItemStack *o = checkObject<LuaItemStack>(L, 1);
	o->m_stack = read_item(L, 2, getCrateDef(L)->idef());
	lua_pushboolean(L, true);
	return 1;
}
...
int LuaItemStack::l_get_stack_max(lua_State *L)
{
	...
	LuaItemStack *o = checkObject<LuaItemStack>(L, 1);
	ItemStack &item = o->m_stack;
	lua_pushinteger(L, item.getStackMax(getCrateDef(L)->idef()));
	return 1;
}
...
int LuaItemStack::l_get_free_space(lua_State *L)
{
	...
	LuaItemStack *o = checkObject<LuaItemStack>(L, 1);
	ItemStack &item = o->m_stack;
	lua_pushinteger(L, item.freeSpace(getCrateDef(L)->idef()));
	return 1;
}
...
int LuaItemStack::l_is_known(lua_State *L)
{
	...
	LuaItemStack *o = checkObject<LuaItemStack>(L, 1);
	ItemStack &item = o->m_stack;
	bool is_known = item.isKnown(getCrateDef(L)->idef());
...
int LuaItemStack::l_get_tool_capabilities(lua_State *L)
{
	...
	const ToolCapabilities &prop =
		item.getToolCapabilities(getCrateDef(L)->idef());
...
int LuaItemStack::l_add_wear(lua_State *L)
{
	...
	bool result = item.addWear(amount, getCrateDef(L)->idef());
...
int LuaItemStack::l_add_wear_by_uses(lua_State *L)
{
	...
	bool result = item.addWear(add_wear, getCrateDef(L)->idef());
	lua_pushboolean(L, result);
	return 1;
}
...
int LuaItemStack::l_get_wear_bar_params(lua_State *L)
{
	...
	LuaItemStack *o = checkObject<LuaItemStack>(L, 1);
	ItemStack &item = o->m_stack;
	auto params = item.getWearBarParams(getCrateDef(L)->idef());
	if (params.has_value()) {
...
int LuaItemStack::l_add_item(lua_State *L)
{
	...
	LuaItemStack *o = checkObject<LuaItemStack>(L, 1);
	ItemStack &item = o->m_stack;
	ItemStack newitem = read_item(L, -1, getCrateDef(L)->idef());
	ItemStack leftover = item.addItem(newitem, getCrateDef(L)->idef());
	create(L, leftover);
	return 1;
}
...
int LuaItemStack::l_item_fits(lua_State *L)
{
	...
	LuaItemStack *o = checkObject<LuaItemStack>(L, 1);
	ItemStack &item = o->m_stack;
	ItemStack newitem = read_item(L, 2, getCrateDef(L)->idef());
	ItemStack restitem;
	bool fits = item.itemFits(newitem, &restitem, getCrateDef(L)->idef());
...
int LuaItemStack::create_object(lua_State *L)
{
	...
	ItemStack item;
	if (!lua_isnone(L, 1))
		item = read_item(L, 1, getCrateDef(L)->idef());
...
int ModApiItem::l_get_content_id(lua_State *L)
{
	...
	const IItemDefManager *idef = getCrateDef(L)->idef();
	const NodeDefManager *ndef = getCrateDef(L)->ndef();
