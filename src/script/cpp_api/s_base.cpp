...
void ScriptApiBase::checkSetByBuiltin()
{
	...
	if (getType() == ScriptingType::Server ||
			(getType() == ScriptingType::Async && m_cratedef) ||
...
{
	// Since the cratedef is the server it's still possible to retrieve it in
	// e.g. the async environment, but this isn't meant to happen.
	// TODO: still needs work
	//assert(getType() == ScriptingType::Server);
	return dynamic_cast<Server *>(m_cratedef);
}

#if CHECK_CLIENT_BUILD()
Client *ScriptApiBase::getClient()
{
	return dynamic_cast<Client *>(m_cratedef);
}
