...
public:
	...
	/*
	 * Материалы нод, которые рисуются спрайтом, а не кубом: трава, цветы,
	 * огонь.
	 *
	 * Сплошное поле такой травы — это и миллионы вершин, и перерисовка в
	 * несколько слоёв: взгляд вдоль земли проходит сквозь десяток травинок,
	 * и каждая считается полным шейдером. Дальнюю траву за это платить не
	 * стоит, и чтобы её отсечь, надо уметь её узнавать.
	 */
	std::vector<u32> m_sprite_materials;
#endif
