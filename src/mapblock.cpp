...
MapBlock::MapBlock(v3s16 pos, ICrateDef *cratedef):
		m_pos(pos),
...
void MapBlock::reallocate(u32 count, MapNode n)
{
	...
	assert(!m_cratedef->isClient() || count == nodecount);
		...
		ICrateDef *cratedef)
{
	...
	for (u32 i = 0; i < MapBlock::nodecount; i++) {
		...
		if (!nodedef->getId(name, global_id)) {
			global_id = cratedef->allocateUnknownNodeId(name);
...
void MapBlock::serialize(std::ostream &os_compressed, u8 version, bool disk, int compression_level)
{
	...
	*/
	...
	if(disk)
	{
		...
		getBlockNodeIdMapping(&nimap, tmp_nodes.get(), size, m_cratedef->ndef());
...
void MapBlock::deSerialize(std::istream &in_compressed, u8 version, bool disk)
{
	...
	*/
	...
	if (version >= 29) {
		m_node_metadata.deSerialize(is, m_cratedef->idef());
	} else {
		try {
			// reuse in_raw
			in_raw.str("");
			...
			decompress(is, in_raw, version);
			if (version >= 23)
				m_node_metadata.deSerialize(in_raw, m_cratedef->idef());
			else
	...
	*/
	if (disk) {
		...
		correctBlockNodeIds(&nimap, data, m_cratedef);
