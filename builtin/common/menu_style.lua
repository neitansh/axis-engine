...
-- Поля содержимого внутри карточки: на этом отступе стоят карточки экранов,
-- на нём же — черта и подпись подвала. Одна величина на всех, чтобы края
-- сходились по вертикали.
menu_style.PAD = 0.375
...
--- Поле ввода без тёмной врезки: лёгкая заливка и черта снизу. Тёмный
--- прямоугольник посреди светлого списка кричит громче самого поля.
function menu_style.field(x, y, w, h)
	return ("box[%f,%f;%f,%f;#FFFFFF0A]box[%f,%f;%f,0.03;#FFFFFF22]"):format(
		x, y, w, h, x, y + h - 0.03, w)
end
