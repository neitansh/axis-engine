...
-- Отмечает строку к переводу, ничего не переводя.
--
-- Нужна там, где строку объявляют в одном месте, а переводят в другом: подпись
-- вкладки живёт в её описании, а переводится при сборке страницы, чтобы смена
-- языка была видна сразу. Сборщику строк (`util/extract_pot.sh`) видно только
-- то, что стоит прямо в вызове, — без этой отметки подпись не попадала в
-- шаблон вовсе, и перевода у неё не появлялось никогда. Так и остались
-- английскими «Play», «About» и «Singleplayer».
function N_(text)
	return text
end
